...
function* handleFetchEventApplicantsRequest(
	action: ReturnType<typeof fetchEventApplicantsRequest>,
) {
	try {
...
function* watchFetchEventApplicantsRequest() {
	yield takeEvery(
		ApplicationsTypes.FETCH_EVENT_APPLICANTS,
		handleFetchEventApplicantsRequest,
	);
}

function* handleFetchMineApplicationsRequest(
	action: ReturnType<typeof fetchMineApplicationsRequest>,
) {
	try {
...
function* watchFetchMineApplicationsRequest() {
	yield takeEvery(
		ApplicationsTypes.FETCH_MINE_APPLICATIONS,
		handleFetchMineApplicationsRequest,
	);
}

function* handleFetchApplicationsToMeRequest(
	action: ReturnType<typeof fetchApplicationsToMeRequest>,
) {
	try {
		const vkUserId = yield select(getVkUserId);
...
function* watchFetchApplicationsToMeRequest() {
	yield takeEvery(
		ApplicationsTypes.FETCH_APPLICATIONS_TO_ME,
		handleFetchApplicationsToMeRequest,
	);
}

function* handleApplyToEventFromUserEvents(
	action: ReturnType<typeof applyToEventFromUserEvents>,
) {
	yield put(showSpinner());
	yield put(applyToEventRequest(action.payload));
	var effect: Action = yield take(applyToEventRequest);
...
function* watchApplyToEventFromUserEvents() {
	yield takeLatest(
		ApplicationsTypes.APPLY_TO_EVENT_FROM_USERS_EVENTS,
		handleApplyToEventFromUserEvents,
	);
}

function* handleApplyToEventFromEvents(
	action: ReturnType<typeof applyToEventFromEvents>,
) {
	yield put(showSpinner());
	yield put(applyToEventRequest(action.payload));
	var effect: Action = yield take(applyToEventRequest);
...
function* watchApplyToEventFromEvents() {
	yield takeLatest(
		ApplicationsTypes.APPLY_TO_EVENT_FROM_EVENTS,
		handleApplyToEventFromEvents,
	);
}

function* handleApplyToEventFromSelectedEvent(
	action: ReturnType<typeof applyToEventFromSelectedEvent>,
) {
	yield put(showSpinner());
	yield put(applyToEventRequest(action.payload));
	var effect: Action = yield take(applyToEventRequest);
...
function* watchApplyToEventFromSelectedEvent() {
	yield takeLatest(
		ApplicationsTypes.APPLY_TO_EVENT_FROM_SELECTED_EVENT,
		handleApplyToEventFromSelectedEvent,
	);
}

function* handleApplyToEventRequest(
	action: ReturnType<typeof applyToEventRequest>,
) {
	try {
		const vkUserId = yield select(getVkUserId);
...
function* handleConfirmApplicantRequest(
	action: ReturnType<typeof confirmApplicantRequest>,
) {
	try {
		yield put(showSpinner());
		const vkUserId = yield select(getVkUserId);
...
function* watchConfirmApplicantRequest() {
	yield takeLatest(
		ApplicationsTypes.CONFIRM_APPLICANT,
		handleConfirmApplicantRequest,
	);
}

function* handleRejectApplicantRequest(
	action: ReturnType<typeof rejectApplicantRequest>,
) {
	try {
		yield put(showSpinner());
		const vkUserId = yield select(getVkUserId);
...
function* watchRejectApplicantRequest() {
	yield takeLatest(
		ApplicationsTypes.REJECT_APPLICANT,
		handleRejectApplicantRequest,
	);
}

function* handleRevokeApplicationRequest(
	action: ReturnType<typeof revokeApplicationRequest>,
) {
	try {
		yield put(showSpinner());
		const vkUserId = yield select(getVkUserId);
...
function* watchRevokeApplicationRequest() {
	yield takeLatest(
		ApplicationsTypes.REVOKE_APPLICATION,
		handleRevokeApplicationRequest,
	);
}
