...
const reducer: Reducer<ApplicationsState, ApplicationsActions> = (
	state = initialState,
	...
	switch (action.type) {
		...
		case ApplicationsTypes.FETCH_MINE_APPLICATIONS_SUCCESS: {
			return {
				...state,
				mineApplications: action.payload,
			};
		}
		case ApplicationsTypes.FETCH_APPLICATIONS_TO_ME_SUCCESS: {
			return {
				...state,
				applicationsToMe: action.payload,
			};
		}
			...
			return {
				...
				eventsApplicants: {
					...
					[action.payload.eventId]: [
						...state.eventsApplicants[action.payload.eventId].filter(
							(m) => m.id !== action.payload.applicationId,
						),
					],
				},
			};
