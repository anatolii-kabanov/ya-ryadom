...
import { ComplaintToEventRequest } from './models';
...
function* handleSendComplaintToEventRequest(
	action: ReturnType<typeof sendComplaintToEventRequest>,
) {
	try {
...
function* watchSendComplaintToEventRequest() {
	yield takeLatest(
		ComplaintsTypes.SEND_COMPLAINT_TO_EVENT,
		handleSendComplaintToEventRequest,
	);
}

function* handleSendComplaintToUserRequest(
	action: ReturnType<typeof sendComplaintToUserRequest>,
) {
	try {
		yield put(showSpinner());
		const vkUserId = yield select(getVkUserId);
...
function* watchSendComplaintToUserRequest() {
	yield takeLatest(
		ComplaintsTypes.SEND_COMPLAINT_TO_USER,
		handleSendComplaintToUserRequest,
	);
}

function* handleOpenEventComplaintForm(
	action: ReturnType<typeof openEventComplaintForm>,
) {
...
function* watchOpenEventComplaintForm() {
	yield takeLatest(
		ComplaintsTypes.OPEN_EVENT_COMPLAINT_FORM,
		handleOpenEventComplaintForm,
	);
}
