...
import {
	...
	addReviewSuccess,
	setUserToReview,
	fetchReviewsAboutUserRequest,
	fetchReviewsAboutUserError,
	fetchReviewsAboutUserSuccess,
} from './actions';
...
function* handleFetchEventReviewsRequest(
	action: ReturnType<typeof fetchEventReviewsRequest>,
) {
	try {
...
function* watchFetchEventReviewsRequest() {
	yield takeEvery(
		ReviewsTypes.FETCH_EVENT_REVIEWS,
		handleFetchEventReviewsRequest,
	);
}
...
function* handleFetchReviewsAboutUserRequest(
	action: ReturnType<typeof fetchReviewsAboutUserRequest>,
) {
	try {
		yield put(showSpinner());
		...
		if (result.errors) {
			...
			yield put(
				fetchReviewsAboutUserSuccess({ vkUserId: vkUserId, reviews: result }),
			);
		}
	} catch (error) {
		if (error instanceof Error && error.stack) {
...
function* watchFetchReviewsAboutUserRequest() {
	yield takeEvery(
		ReviewsTypes.FETCH_REVIEWS_ABOUT_USER,
		handleFetchReviewsAboutUserRequest,
	);
}
