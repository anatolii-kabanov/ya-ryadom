...
const reducer: Reducer<ReviewsState, ReviewsActions> = (state = initialState, action: ReviewsActions) => {
    switch (action.type) {
        ...
        case ReviewsTypes.FETCH_REVIEWS_ABOUT_USER_SUCCESS: {
            const newState = _.cloneDeep(state);
            newState.reviewsAboutUsers[action.payload.vkUserId] = action.payload.reviews;
            return newState;
        }
