...
function* handleFetchEventByIdRequest(
...
function* watchFetchEventByIdRequest() {
	yield takeEvery(
		EventsNearMeTypes.FETCH_EVENT_BY_ID,
		handleFetchEventByIdRequest,
	);
}

function* eventsNearMeSagas() {
	yield all([fork(watchFetchRequest), fork(watchFetchEventByIdRequest)]);
}
