...
const reducer: Reducer<EventsNearMeState, EventsNearMeActions> = (
	state = initialState,
	action: EventsNearMeActions,
) => {
	switch (action.type) {
		...
		case EventsNearMeTypes.SET_SENT_STATUS: {
			const newState = _.cloneDeep(state);
			const index = newState.eventsList.findIndex(
		...
		case EventsNearMeTypes.SET_SHARED_EVENT_SENT_STATUS: {
			const newState = _.cloneDeep(state);
			newState.sharedEvents[action.payload].applicationStatus =
