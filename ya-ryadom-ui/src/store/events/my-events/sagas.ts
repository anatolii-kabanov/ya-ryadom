...
function* handleSaveEventRequest(
	action: ReturnType<typeof saveMyEventRequest>,
) {
	try {
		yield put(showSpinner());
		const result = yield call(
...
function* handleSaveEventIntroRequest(
	action: ReturnType<typeof saveMyEventIntroRequest>,
) {
...
function* watchSaveEventIntroRequest() {
	yield takeLatest(
		MyEventsTypes.SAVE_MY_EVENT_INTRO,
		handleSaveEventIntroRequest,
	);
}

function* handleSaveEventGeneralRequest(
	action: ReturnType<typeof saveMyEventGeneralRequest>,
) {
	yield put(saveMyEventRequest(action.payload));
	yield take(MyEventsTypes.SAVE_MY_EVENT_SUCCESS);
	yield put(goBack());
...
function* watchSaveEventGeneralRequest() {
	yield takeLatest(
		MyEventsTypes.SAVE_MY_EVENT_GENERAL,
		handleSaveEventGeneralRequest,
	);
}

function* handleRevokeEventRequest(
	action: ReturnType<typeof revokeEventRequest>,
) {
	try {
		yield put(showSpinner());
		const vkUserId = yield select(getVkUserId);
