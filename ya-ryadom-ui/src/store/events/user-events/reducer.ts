...
const reducer: Reducer<UserEventsState, UserEventsActions> = (
	state = initialState,
	action: UserEventsActions,
) => {
	switch (action.type) {
		case UserEventsTypes.FETCH_VISITED_EVENTS_SUCCESS: {
			const newState = _.cloneDeep(state);
			newState.userVisitedEvents[action.payload.vkUserId] =
				action.payload.events;
			return newState;
		}
		case UserEventsTypes.FETCH_CREATED_EVENTS_SUCCESS: {
			const newState = _.cloneDeep(state);
			newState.userCreatedEvents[action.payload.vkUserId] =
				action.payload.events;
			return newState;
		}
		case UserEventsTypes.SET_SENT_STATUS: {
			const newState = _.cloneDeep(state);
			const event = newState.userCreatedEvents[action.payload.vkUserId].find(
				(a) => a.id === action.payload.eventId,
			);
			let applicant = event?.participants.find(
				(p) => p.vkUserId === action.payload.vkUserId,
			);
			if (applicant) {
