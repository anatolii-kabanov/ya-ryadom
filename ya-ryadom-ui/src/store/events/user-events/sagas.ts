...
function* handleUserCreatedEventsFetch(action: ReturnType<typeof fetchUserCreatedEventsListRequest>) {
    try {
        ...
        const result = yield call(callApi, 'get', API_ENDPOINT, `/created/${selectedVkUserId}`);

        if (result.errors) {
            yield put(fetchUserCreatedEventsListError(result.errors));
        } else {
            yield put(fetchUserCreatedEventsListSuccess({ vkUserId: selectedVkUserId, events: result }));
        }
    } catch (error) {
...
function* handleUserVisitedEventsFetch(action: ReturnType<typeof fetchUserVisitedEventsListRequest>) {
    try {
        ...
        const result = yield call(callApi, 'get', API_ENDPOINT, `/visited/${selectedVkUserId}`);

        if (result.errors) {
            yield put(fetchUserVisitedEventsListError(result.errors));
        } else {
            yield put(fetchUserVisitedEventsListSuccess({ vkUserId: selectedVkUserId, events: result }));
        }
    } catch (error) {
        if (error instanceof Error && error.stack) {
