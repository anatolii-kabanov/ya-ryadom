...
import { FormLayout, Button, Title, Div, FormStatus } from '@vkontakte/vkui';
	...
	render() {
		...
		return (
			<Div className='themes-form'>
				<Div>
					<Title level='3' weight='bold' className='title'>
						Выберите темы
					</Title>
				</Div>
				<Div className='pills'>{this.renderThemePills()}</Div>
				{errors?.selectedThemes && (
					<FormStatus mode='error'>{errors?.selectedThemes}</FormStatus>
				)}
				<FormLayout>
					<Button
						className='btn-primary'
						size='xl'
						onClick={this.onFillInProfile}
					>
						{btnText ?? 'Продолжить'}
					</Button>
				</FormLayout>
