...
class RootLayout extends React.Component<AllProps> {
		...
		if (!window.navigator.onLine) {
			addNotification(
				new SnackbarErrorNotification(
					NOTIFICATION_MESSAGES.CHECK_INTERNET_CONNECTION,
				),
			);
		}
	};
	...
	renderLayout() {
		...
		switch (activeView) {
			case VIEWS.INTRO_VIEW:
				return (
					<Root
						...
						popout={spinnerVisible && <ScreenSpinner />}
					>
			...
			default:
				return (
					<MainEpic activeStory={activeView}>
						<ApplicationsView
							id={VIEWS.APPLICATIONS_VIEW}
							popout={spinnerVisible && <ScreenSpinner />}
						></ApplicationsView>
						<ReviewsView
							id={VIEWS.REVIEWS_VIEW}
							popout={spinnerVisible && <ScreenSpinner />}
						></ReviewsView>
						<MyProfileView
							id={VIEWS.MY_PROFILE_VIEW}
							popout={spinnerVisible && <ScreenSpinner />}
						></MyProfileView>
						<EventsView
							id={VIEWS.EVENTS_NEAR_ME_VIEW}
							popout={spinnerVisible && <ScreenSpinner />}
						></EventsView>
						<MyEventCreateView
							id={VIEWS.MY_EVENT_CREATE_VIEW}
							popout={spinnerVisible && <ScreenSpinner />}
						></MyEventCreateView>
					</MainEpic>
				);
	...
	render() {
		...
		return (
			<ConfigProvider
				isWebView={isWebView}
				scheme={vkStyles.schemeType}
				appearance={vkStyles.appearance}
			>
				{!currentUser ? (
					<ScreenSpinner />
				) : (
