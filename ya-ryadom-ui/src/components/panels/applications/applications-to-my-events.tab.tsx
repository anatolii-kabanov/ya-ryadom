				...
				.map((item, key) => {
					return (
						<RichCell
							...
							caption={`${new Date(item.sentDate).toLocaleDateString('ru-RU', {
								month: 'long',
								day: 'numeric',
								hour12: false,
							})}`}
							actions={
								<span className='application-btns'>
									<Button
										...
										onClick={() =>
											confirm({ applicationId: item.id, eventId: id })
										}
									>
										Принять
									</Button>
									<Button
										...
										onClick={() =>
											reject({ applicationId: item.id, eventId: id })
										}
									>
										Отклонить
									</Button>
		...
		if (myEvents) {
			return myEvents.map((item, index) => {
				const photos = item.participants
					.filter((p) => p.applicationStatus === ApplicationStatus.sent)
				...
				return (
					<Group
						key={index}
						separator={index !== myEvents.length - 1 ? 'show' : 'hide'}
						...
						header={
							<Header
								...
								aside={
									!item.ended && (
										<Icon16MoreHorizontal onClick={() => openBase(item.id)} />
									)
								}
							>
