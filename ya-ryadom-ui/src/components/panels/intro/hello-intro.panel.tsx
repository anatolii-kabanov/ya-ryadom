...
import { Panel, PanelHeader, Group, Div, Button, Title } from '@vkontakte/vkui';
...
class HelloIntroPanel extends React.Component<AllProps> {
	...
	onHelloIntroCompleted() {
		...
		goForwardView(
			new VkHistoryModel(VIEWS.INTRO_VIEW, PANELS.GEOLOCATION_INTRO_PANEL),
		);
	}
	...
	render() {
		...
		return (
			<Panel id={id} className='hello-intro-panel'>
				...
				<Title level='2' weight='semibold' className='title'>
				...
				<Group className='intro-group' separator='hide'>
					...
					<Div className='intro-text'>
						<p className='first-row-text'>
							Твой будущий друг уже рядом. <br /> Несколько действий и ты в
							большой тусовке своего города.
						</p>
						<p className='second-row-text'>
							Весь спектр интересов в одном приложении. <br /> Каждый найдёт
							себе дело по душе.
						</p>
						<Button
							size='xl'
							className='btn-primary'
							onClick={() => this.onHelloIntroCompleted()}
						>
							Начать
						</Button>
					</Div>
...
export default connect(mapStateToProps, mapDispatchToProps)(HelloIntroPanel);
