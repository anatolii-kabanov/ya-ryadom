...
class UserProfilePanel extends React.Component<AllProps, State> {
	...
	private renderThemes() {
		...
		if (selectedUserInfo?.selectedThemes) {
			return selectedUserInfo?.selectedThemes.map((themeId, key) => {
			...
			<Panel className='my-profile' id={id}>
				...
				<Group separator='hide'>
					<RichCell
						...
						before={
							<Avatar size={72} src={selectedUserInfo?.vkUserAvatarUrl} />
						}
					>
						<span className='profile-main-row'>
							...
							<Caption className='rating' weight='regular' level='1'>
								{selectedUserInfo?.avgRating.toFixed(1)}
							</Caption>
						</span>
					</RichCell>
				</Group>
					...
					header={
						<Header mode='secondary'>
							<span className='themes-text'>
								Темы{' '}
								<span className='themes-orange-text'>
									у вас{' '}
									{
										selectedUserInfo?.selectedThemes.filter((t) =>
											currentUser?.selectedThemes.includes(t),
										).length
									}{' '}
									общих
								</span>
...
const mapStateToProps = (
	{ events, authentication, users }: AppState,
	ownProps: OwnProps,
) => ({
...
export default connect(mapStateToProps, mapDispatchToProps)(UserProfilePanel);
